...
export const BADGE_DEFINITIONS = [
  ...
  // ----- Skills -----
  {
    slug: 'first-step',
    name: 'Pierwszy krok',
    description: 'Dowolna umiejętność osiągnęła poziom 2.',
    category: 'skills',
    icon: 'firstStep',
    async check(playerId) {
      const player = await Player.findById(playerId).select('skills').lean();
      if (!player) return { earned: false, progress: { current: 0, target: 1 } };
      const skillNames = ['serve', 'forehand', 'backhand', 'volley', 'movement', 'tactics', 'mental', 'fitness'];
      const max = Math.max(...skillNames.map(k => player.skills?.[k]?.score ?? 0));
      return { earned: max >= 2, progress: { current: max >= 2 ? 1 : 0, target: 1 } };
    },
  },
  {
    slug: 'practitioner',
    name: 'Praktyk',
    description: 'Dowolna umiejętność osiągnęła poziom 3.',
    category: 'skills',
    icon: 'practitioner',
    async check(playerId) {
      const player = await Player.findById(playerId).select('skills').lean();
      ...
      const skillNames = ['serve', 'forehand', 'backhand', 'volley', 'movement', 'tactics', 'mental', 'fitness'];
      const max = Math.max(...skillNames.map(k => player.skills?.[k]?.score ?? 0));
      return { earned: max >= 3, progress: { current: max >= 3 ? 1 : 0, target: 1 } };
    },
  },
  {
    slug: 'stable-player',
    name: 'Stabilny zawodnik',
    description: 'Dowolna umiejętność osiągnęła poziom 4.',
    category: 'skills',
    icon: 'stablePlayer',
    async check(playerId) {
      const player = await Player.findById(playerId).select('skills').lean();
      ...
      const skillNames = ['serve', 'forehand', 'backhand', 'volley', 'movement', 'tactics', 'mental', 'fitness'];
      const max = Math.max(...skillNames.map(k => player.skills?.[k]?.score ?? 0));
      return { earned: max >= 4, progress: { current: max >= 4 ? 1 : 0, target: 1 } };
    },
  },
  {
    slug: 'strong-skill',
    name: 'Silna umiejętność',
    description: 'Dowolna umiejętność osiągnęła maksymalny poziom 5.',
    category: 'skills',
    icon: 'strongSkill',
    async check(playerId) {
      const player = await Player.findById(playerId).select('skills').lean();
      ...
      const skillNames = ['serve', 'forehand', 'backhand', 'volley', 'movement', 'tactics', 'mental', 'fitness'];
      const max = Math.max(...skillNames.map(k => player.skills?.[k]?.score ?? 0));
      return { earned: max >= 5, progress: { current: max >= 5 ? 1 : 0, target: 1 } };
    },
  },
  {
    slug: 'all-rounder',
    name: 'Wszechstronny',
    description: 'Co najmniej 4 umiejętności na poziomie 3 lub wyżej.',
    category: 'skills',
    icon: 'allRounder',
    async check(playerId) {
      const player = await Player.findById(playerId).select('skills').lean();
      if (!player) return { earned: false, progress: { current: 0, target: 4 } };
      const skillNames = ['serve', 'forehand', 'backhand', 'volley', 'movement', 'tactics', 'mental', 'fitness'];
      const count = skillNames.filter(k => (player.skills?.[k]?.score ?? 0) >= 3).length;
      return { earned: count >= 4, progress: { current: Math.min(count, 4), target: 4 } };
    },
  },
