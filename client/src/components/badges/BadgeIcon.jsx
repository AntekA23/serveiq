...
function BadgeShell({ earned, color, size = 64, children }) {
  ...
  return (
    ...
    >
      ...
      {/* Inner highlight */}
      <ellipse cx="32" cy="24" rx="14" ry="8" fill="white" fillOpacity="0.06" />
      {/* Children (icon graphics) */}
      {children}
      {/* Shine when earned */}
      {earned && (
        <circle cx="22" cy="20" r="4" fill="white" fillOpacity="0.15" />
      )}
    </svg>
  )
}
...
function FirstSession({ earned, size }) {
  const c = earned ? CATEGORY_COLORS.training : LOCKED_COLOR
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.training} size={size}>
      {/* Ball body */}
      <circle cx="32" cy="32" r="13" fill={c} fillOpacity="0.9" />
      {/* Seam lines */}
      ...
      {/* Green felt color overlay */}
      <circle cx="32" cy="32" r="13" fill="#22c55e" fillOpacity={earned ? 0.35 : 0.15} />
    </BadgeShell>
  )
}
...
function RegularPlayer({ earned, size }) {
  ...
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.training} size={size}>
      ...
      {/* Strings horizontal */}
      <line x1="22" y1="24" x2="42" y2="24" stroke={c} strokeWidth="1" opacity="0.7" />
      <line x1="22" y1="28" x2="42" y2="28" stroke={c} strokeWidth="1" opacity="0.7" />
      <line x1="22" y1="32" x2="42" y2="32" stroke={c} strokeWidth="1" opacity="0.7" />
      {/* Strings vertical */}
      <line x1="27" y1="16" x2="27" y2="38" stroke={c} strokeWidth="1" opacity="0.7" />
      <line x1="32" y1="16" x2="32" y2="38" stroke={c} strokeWidth="1" opacity="0.7" />
      <line x1="37" y1="16" x2="37" y2="38" stroke={c} strokeWidth="1" opacity="0.7" />
      {/* Handle */}
...
function WeeklyStreak({ earned, size }) {
  ...
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.training} size={size}>
      ...
      {/* Calendar top bar */}
      <rect x="18" y="20" width="28" height="7" rx="3" fill={c} fillOpacity="0.5" />
      {/* Hook lines */}
...
function StreakMaster({ earned, size }) {
  const c = earned ? CATEGORY_COLORS.training : LOCKED_COLOR
  const outerFlame = earned ? '#FCD34D' : LOCKED_COLOR
  const innerFlame = earned ? '#EF4444' : '#6b7280'
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.training} size={size}>
      {/* Outer flame */}
      <path
...
function FirstStep({ earned, size }) {
  const c = earned ? CATEGORY_COLORS.skills : LOCKED_COLOR
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.skills} size={size}>
      {/* Foot body */}
      <path
        d="M28 22 C25 22 22 26 22 31 C22 36 25 42 30 42 C35 42 38 37 38 32 C38 27 35 22 31 22 Z"
        fill={c}
        fillOpacity="0.8"
      />
      {/* Toe circles */}
      <circle cx="25" cy="20" r="2.5" fill={c} fillOpacity="0.8" />
      <circle cx="29" cy="18" r="2.5" fill={c} fillOpacity="0.8" />
      <circle cx="33" cy="18" r="2.5" fill={c} fillOpacity="0.8" />
      <circle cx="37" cy="20" r="2" fill={c} fillOpacity="0.8" />
    </BadgeShell>
  )
}
...
function StablePlayer({ earned, size }) {
  ...
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.skills} size={size}>
      {/* Shield */}
      <path
        d="M32 16 L44 21 L44 32 C44 38 38 43 32 46 C26 43 20 38 20 32 L20 21 Z"
        stroke={c}
        strokeWidth="2"
        fill={c}
        fillOpacity="0.2"
...
function StrongSkill({ earned, size }) {
  const c = earned ? CATEGORY_COLORS.skills : LOCKED_COLOR
  const boltFill = earned ? '#FCD34D' : '#6b7280'
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.skills} size={size}>
      <path
        d="M36 16 L26 33 L32 33 L28 48 L42 28 L36 28 Z"
        fill={boltFill}
        stroke={c}
        strokeWidth="1.5"
...
function AllRounder({ earned, size }) {
  ...
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.skills} size={size}>
      <path
        d="M32 16 L34 28 L46 32 L34 36 L32 48 L30 36 L18 32 L30 28 Z"
        fill={c}
        fillOpacity="0.85"
        stroke={c}
        strokeWidth="1"
...
function TournamentDebut({ earned, size }) {
  const c = earned ? CATEGORY_COLORS.tournament : LOCKED_COLOR
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.tournament} size={size}>
      {/* Trophy cup */}
      <path
        d="M24 16 L24 34 C24 40 28 44 32 44 C36 44 40 40 40 34 L40 16 Z"
        stroke={c}
        strokeWidth="2"
        fill={c}
        fillOpacity="0.25"
...
function Winner({ earned, size }) {
  const c = earned ? CATEGORY_COLORS.tournament : LOCKED_COLOR
  const starFill = earned ? '#FCD34D' : '#6b7280'
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.tournament} size={size}>
      {/* Ribbon left */}
      <path d="M26 16 L22 28 L28 25 Z" fill={c} fillOpacity="0.7" />
      {/* Ribbon right */}
      <path d="M38 16 L42 28 L36 25 Z" fill={c} fillOpacity="0.7" />
      {/* Medal circle */}
      <circle cx="32" cy="36" r="12" fill={c} fillOpacity="0.25" stroke={c} strokeWidth="2" />
      {/* Star inside medal */}
      <path
        d="M32 29 L33.5 33.5 L38 33.5 L34.5 36.5 L36 41 L32 38.5 L28 41 L29.5 36.5 L26 33.5 L30.5 33.5 Z"
        fill={starFill}
      />
    </BadgeShell>
  )
...
function GoalAchieved({ earned, size }) {
  const c = earned ? CATEGORY_COLORS.special : LOCKED_COLOR
  return (
    <BadgeShell earned={earned} color={CATEGORY_COLORS.special} size={size}>
      ...
      <circle cx="32" cy="32" r="3.5" fill={c} fillOpacity="0.9" />
