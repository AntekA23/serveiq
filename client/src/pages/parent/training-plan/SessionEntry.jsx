...
import { SESSION_TYPES, SURFACES, formatDuration } from './constants'
    ...
    <div className={`tp-entry ${compact ? 'compact' : ''}`} style={{ '--entry-color': typeInfo.color, '--entry-bg': typeInfo.bg }}>
      ...
      <div className="tp-entry-body">
        <div className="tp-entry-top">
          ...
          {session.surface && SURFACES[session.surface] && (
            <span className="tp-entry-surface">{SURFACES[session.surface].emoji} {SURFACES[session.surface].label}</span>
          )}
